#!/usr/bin/python
#
# Tests for the SLA patch which adds the head_msg_timestamp queue stat.
...
class RabbitSlaTestCase(RabbitTestCase):
    ...
    def get_head_msg_timestamp(self, queue_name):
        return self.get_queue_stats(queue_name)["backing_queue_status"]["head_msg_timestamp"]
    ...
    def wait_for_new_timestamp(self, queue, old_timestamp):
        ...
        while ((clock() - stats_wait_start) < TIMEOUT_SECS and
               self.get_head_msg_timestamp(queue) == old_timestamp):
            sleep(0.1)
        log('Queue stats updated in ' + str(clock() - stats_wait_start) + ' secs.')
        return self.get_head_msg_timestamp(queue)

    # TESTS

    def test_no_timestamp_when_queue_is_empty(self):
        assert self.get_head_msg_timestamp(TQUEUE) == 'undefined'

    def test_has_timestamp_when_first_msg_is_added(self):
        self.send('Msg1', TIMESTAMP1)
        stats_timestamp = self.wait_for_new_timestamp(TQUEUE, 'undefined')
    ...
    def test_no_timestamp_when_last_msg_is_removed(self):
        ...
        assert stats_timestamp == 'undefined'
    ...
    def test_timestamp_not_updated_before_msg_is_acked(self):
        ...
        assert self.get_head_msg_timestamp(TQUEUE) == TIMESTAMP1
        self.ack(tag) 
